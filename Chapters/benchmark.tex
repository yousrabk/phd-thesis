...
Figure~\ref{all_solvers_amp_error} \alex{?? fig} shows the errors in amplitude. The most important point in this figure is to see the difference between the convex MxNE and the non-convex irMxNE in terms of amplitude bias. All dipoles basically improve their amplitude estimate when using the non-convex method (irMxNE). For some cases, the irMxNE amplitude estimate is even better than the dipole fitting one.
