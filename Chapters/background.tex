...
Maxwell's equations are a set of partial differential equations forming the foundation of classical electromagnetism. We denote by $\mathbf{E}$ by the electric field, $\mathbf{B}$ the magnetic field, $\mathbf{J}$ the current density, and $\rho$ the charge density.
...
Considering a head as a finite three dimensional volume conductor, and non magnetic ($\mu = \mu_0$...
...
% On the subsequent chapter \ref{chapter:multiscale}, we will go further into details to explain each contribution.
