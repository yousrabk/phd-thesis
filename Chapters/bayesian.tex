...
Due to Slater's conditions~\cite{Boyd_Vandenberghe04}, the strong duality gap holds for the MxNE subproblem to check the convergence of Equation~\eqref{eq:MM}. Based on Fenchel-Rockafellar duality theorem~\cite{rockafellar:1997}, the dual objective function associated with the primal objective function:
\begin{equation}
\begin{split}
...
\While{
	  ...
	  }
	  {
	  	...
	  	$\mathbf{X}^{\star\Gamma}\leftarrow$ Use Algorithm~\ref{alg:mxne_bcd} with $\mathbf{\mu}, \mathbf{G}^\Gamma$, and $\mathbf{X}_0=\mathbf{X}^\Gamma$
